"""silver/emails.jsonl -> gold.db (SQLite: metadata columns + FTS5 + sqlite-vec), and hybrid search.
  ...
  python gold.py search "query" [--from x] [--since 2020-01-01] [--until ...] [--human] [-n 10]
"""
...
def connect():
    ...
    c.executescript(f'''
      ...
      create table if not exists emails(
        ...
        is_machine integer, attachments text);
...
def build(limit=None, rebuild=False):
    ...
    for r, v in zip(new, vecs):
        f = r['from'][0] if r['from'] else {'name': '', 'addr': ''}
        cur = c.execute('insert into emails(id,thread_id,date,folder,direction,from_addr,from_name,to_addrs,subject,body,body_raw,is_machine,attachments) values(?,?,?,?,?,?,?,?,?,?,?,?,?)',
                        (r['id'], r['thread_id'], r['date'], r['folder'], r['direction'], f['addr'], f['name'],
                         ' '.join(t['addr'] for t in r['to']), r['subject'], r['body'], r['body_raw'], int(r['is_machine']), json.dumps(r['attachments'])))
...
def search(q, n=10, from_=None, since=None, until=None, human=False, k=60):
    c = connect()
    where, args = [], []
    ...
    out = []
    for rowid, s in top:
        e = dict(c.execute('select id,thread_id,date,folder,direction,from_addr,from_name,subject,body,is_machine from emails where rowid=?', (rowid,)).fetchone())
...
if __name__ == '__main__':
    ...
    s.add_argument('--from', dest='from_'); s.add_argument('--since'); s.add_argument('--until'); s.add_argument('--human', action='store_true')
    a = p.parse_args()
    if a.cmd == 'build': build(a.limit, a.rebuild)
    else:
        t = time.time(); hits = search(a.q, a.n, a.from_, a.since, a.until, a.human)
